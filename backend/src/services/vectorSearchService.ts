...
const logger = new Logger('VectorSearchService');
...
export class VectorSearchService {
   ...
   */
  async search(query: string, limit: number = 10, skip: number = 0): Promise<VectorSearchResult[]> {
    try {
   ...
   */
  private async mongoVectorSearch(
    ...
    try {
      ...
      const formattedResults: VectorSearchResult[] = results.map((doc: any) => ({
   ...
   */
  private async applicationVectorSearch(
    queryEmbedding: number[],
    limit: number,
    skip: number
  ): Promise<VectorSearchResult[]> {
    try {
      ...
      const allDocuments = await Resume.aggregate([
        {
          $match: {
            embedding: {
              $exists: true,
      ...
      const scored: Array<VectorSearchResult & { rawScore: number }> = allDocuments.map((doc: any) => {
  ...
  async advancedSearch(
    ...
    try {
      ...
      const scored: Array<VectorSearchResult & { rawScore: number }> = allDocuments
        .map((doc: any) => {
          const similarity = this.computeCosineSimilarity(queryEmbedding, doc.embedding || []);
          return {
            _id: doc._id.toString(),
  ...
  async getSearchStats(): Promise<{
    ...
    try {
      ...
      const totalDocuments = await Resume.countDocuments();

      // Count documents that have embeddings
      const documentsWithEmbeddings = await Resume.countDocuments({
      ...
      logger.info(
        `Stats: total=${totalDocuments}, embedded=${documentsWithEmbeddings}, dimension=${stats.embeddingDimension}`
      );
