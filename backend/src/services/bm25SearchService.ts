...
const logger = new Logger('BM25SearchService');
...
export class BM25SearchService {
   ...
   */
  async search(options: BM25SearchOptions): Promise<{
    ...
    try {
      ...
      logger.info(
        `BM25 search completed: found ${results.length} results (${totalCount} total matches)`
      );
  ...
  async advancedSearch(options: {
    query: string;
    fields?: {
      name?: number; // Weight for name field (0-10)
      role?: number;
      company?: number;
      skills?: number;
      education?: number;
    ...
    try {
      ...
      logger.info(
        `Performing advanced BM25 search: query="${query}", fields=${JSON.stringify(fields)}`
      );
      ...
      logger.info(
        `Advanced BM25 search completed: found ${results.length} results`
      );
  ...
  async getSearchStats(): Promise<{
    ...
    try {
      ...
      const textIndexExists = Object.keys(indexes).some(
        (indexName) => indexes[indexName]?.text !== undefined
      );
      ...
      logger.info(
        `Search stats: ${totalDocuments} documents, text index: ${textIndexExists}`
      );
