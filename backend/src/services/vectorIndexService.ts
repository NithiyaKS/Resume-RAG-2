...
const logger = new Logger('VectorIndexService');

export interface VectorIndexConfig {
  indexName: string;
  fieldsToEmbed: string[];
  dimension: number;
  metric: string;
}

export class VectorIndexService {
  private cached: VectorIndexConfig | null = null;
   ...
   */
  async getVectorIndexConfig(): Promise<VectorIndexConfig> {
    try {
      // Check cache
      ...
      const vectorIndex = indexes.find((idx: any) => idx.name === indexName);
      ...
      const fieldsToEmbed = this.extractFieldsFromMapping((vectorIndex as any).mappings);

      const config: VectorIndexConfig = {
        indexName: vectorIndex.name,
        fieldsToEmbed,
        dimension: parseInt(process.env.VECTOR_DIMENSION || '1024'),
        metric: (vectorIndex as any).mappings?.vectorSearch?.metric || 'cosine',
      };
   ...
   */
  private extractFieldsFromMapping(mappings: any): string[] {
    try {
      const fields: string[] = [];
   ...
   */
  private getDefaultConfig(): VectorIndexConfig {
    return {
