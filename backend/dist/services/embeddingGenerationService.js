...
exports.EmbeddingGenerationService = void 0;
const axios_1 = __importDefault(require("axios"));
const vectorIndexService_1 = require("./vectorIndexService");
const logger_1 = require("../utils/logger");
const logger = new logger_1.Logger('EmbeddingGenerationService');
class EmbeddingGenerationService {
    constructor() {
        this.lastRequestTime = 0; // Track last API call for rate limiting
        this.minRequestInterval = 200; // Minimum 200ms between requests (5 req/sec)
        ...
        this.vectorIndexService = new vectorIndexService_1.VectorIndexService();
    ...
    async generateEmbeddings(texts) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const response = await axios_1.default.post(this.mistralApiUrl, request, {
                    headers: {
                ...
                if (attempt < maxRetries) {
                    ...
                    logger.warn(`[Attempt ${attempt}/${maxRetries}] Embedding generation failed: ${lastError.message}. ` +
     ...
     */
    async generateResumeEmbedding(record) {
        try {
            ...
            const textsToEmbed = indexConfig.fieldsToEmbed.map((field) => {
                const value = this.extractFieldValue(record, field);
                return value;
            });
            logger.info(`Extracting fields for embedding: [${indexConfig.fieldsToEmbed.join(', ')}]`);
            ...
            if (embedding.length !== this.dimension) {
                logger.warn(`Embedding dimension mismatch: got ${embedding.length}, expected ${this.dimension}`);
            }
            logger.info(`Resume embedding generated: dimension=${embedding.length}, fields=[${indexConfig.fieldsToEmbed.join(', ')}]`);
            return {
                embedding,
     ...
     */
    async generateResumeEmbeddings(skillsText, fullText) {
        try {
            ...
            const combinedEmb = skillsEmb.map((val, idx) => (val + textEmb[idx]) / 2);
...
exports.EmbeddingGenerationService = EmbeddingGenerationService;
